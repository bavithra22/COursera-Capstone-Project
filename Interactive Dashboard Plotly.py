...
def build_scatter(site,payload):
    ...
    if site == 'ALL':
	    fig = px.scatter(filtered_dfa,x="Payload Mass (kg)", y="class", color="Booster Version Category", title='Scatter plot between Payload vs. Outcome for All Sites')
    else:
        filtered_dfb = filtered_dfa[filtered_dfa['Launch Site'] == site]
        fig = px.scatter(filtered_dfb,x="Payload Mass (kg)", y="class", color="Booster Version Category", title='Scatter plot between Payload vs. Outcome for ' + site)
    return fig
